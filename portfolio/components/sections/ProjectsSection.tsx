...
function fromDB(p: Project): ProjectDisplay {
  return {
    id: p.id,
    title: p.title,
    subtitle: p.subtitle ?? '',
...
function fromStatic(p: typeof staticProjects[0]): ProjectDisplay {
  return {
    id: String(p.id),
    title: p.title,
    subtitle: p.subtitle,
...
function ProjectCard({ project, index }: { project: ProjectDisplay; index: number }) {
...
export function ProjectsSection() {
  const [projects, setProjects] = useState<ProjectDisplay[]>(staticProjects.map(fromStatic));

  useEffect(() => {
    fetch('/api/projects?featured=true&limit=20')
      .then((r) => r.json())
      .then((data) => {
        if (data.success && data.data.items.length > 0) {
          setProjects(data.data.items.map(fromDB));
        }
      })
      .catch(() => {});
  }, []);

            ...
            {Array.from({ length: days }).map((_, d) => {
              ...
              return (
                <motion.div
                  ...
                  className={`w-3 h-3 rounded-sm ${colors[level]} hover:ring-1 hover:ring-violet-400 cursor-default transition-all`}
                />
              );
            })}
          </div>
...
export function GitHubStatsSection() {
  const [stats, setStats] = useState<GitHubStats>(DEFAULTS);
      ...
      .then((data) => {
        if (data.success) {
          setStats({
            ...
            topLanguages: data.data.topLanguages,
  ...
  const statCards = [
    { icon: GitCommitHorizontal, value: `${stats.totalCommits.toLocaleString()}+`, label: 'Total Commits', color: 'text-violet-400' },
    { icon: Star, value: `${stats.totalStars}+`, label: 'Stars Earned', color: 'text-yellow-400' },
    { icon: GitFork, value: `${stats.totalForks}+`, label: 'Forks', color: 'text-blue-400' },
    { icon: Flame, value: String(stats.streak), label: 'Day Streak', color: 'text-orange-400' },
  ];
    ...
    <SectionWrapper id="github" className="max-w-7xl mx-auto">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        >
          ...
          <ContributionGrid grid={stats.contributions} />
          ...
          <div className="relative mb-5">
            <svg viewBox="0 0 100 100" className="w-32 h-32 mx-auto -rotate-90">
              {(() => {
                ...
                return stats.topLanguages.map((lang, i) => {
                  const el = (
                    <circle
                      key={lang.name}
                      cx="50" cy="50" r="40"
                      ...
                      stroke={colors[i]}
                      strokeWidth="12"
                      strokeDasharray={`${lang.percent} ${100 - lang.percent}`}
